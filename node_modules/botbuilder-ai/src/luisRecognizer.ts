 ...
 */
export interface LuisApplicationsBase {
    /**
...
/**
 * Traditional LUIS application Interface
 */
export interface LuisApplication extends LuisApplicationsBase {
    /**
...
/**
 * LUIS application Interface that implements round-robin style of rotating endpointKeys
 */
export interface LuisApplicationWithRotatingKeys extends LuisApplicationsBase {
    /**
     * Array of endpoint keys for talking to LUIS, each one called in rotating turn
     */
    endpointKeysToRotate: string[];
}
...
export class LuisRecognizer {
    ...
    private luisClients: LuisClient[] = [];
    private luisClientIndex: number = 0;
     ...
     */
    public constructor(application: string, options?: LuisPredictionOptions, includeApiResults?: boolean);
    public constructor(application: LuisApplication|LuisApplicationWithRotatingKeys, options?: LuisPredictionOptions, includeApiResults?: boolean);
    public constructor(application: LuisApplication|LuisApplicationWithRotatingKeys|string, options?: LuisPredictionOptions, includeApiResults?: boolean) {
        let tempApplication;
        if (typeof application === 'string') {
            ...
            tempApplication = {
                ...
                endpointKeysToRotate: parsedQuery['subscription-key'] ? [parsedQuery['subscription-key']] : undefined,
            };
        } else {
            const { applicationId, endpoint } = application;
            tempApplication = {
                applicationId,
                endpoint,
                // Convert to LuisApplicationWithRotatingKeys. Insert keys into endpointKeyRotation array
                endpointKeysToRotate: (application as LuisApplication).endpointKey ? [(application as LuisApplication).endpointKey] : (application as LuisApplicationWithRotatingKeys).endpointKeysToRotate,
            };           
        }
        this.validateLuisApplication(tempApplication);
        this.appId = tempApplication.applicationId;
        ...
        this.createLuisClientsArray(tempApplication);
    }
    /**
    ...
    public recognize(context: TurnContext): Promise<RecognizerResult> {
        ...
        if (!cached) {
            ...
            const currentClientIndex = this.luisClientIndex;

            this.setNextLuisClientIndex();

            return this.luisClients[currentClientIndex].prediction.resolve(
                this.appId, utterance,
                {
                    verbose: this.options.includeAllIntents,
                    customHeaders: {
                        'Ocp-Apim-Subscription-Key': this.luisClients[currentClientIndex].credentials['token'],
    ...
    /**
     * Create client(s) and save to this.luisClients array so we can call them by index later. Each client has unique endpointKey
     */
    private createLuisClientsArray(application: LuisApplicationWithRotatingKeys): void {
        const baseUri: string = application.endpoint || 'https://westus.api.cognitive.microsoft.com';
        application.endpointKeysToRotate.forEach((key) => {
            const creds: msRest.TokenCredentials = new msRest.TokenCredentials(key);
            this.luisClients.push(new LuisClient(creds, baseUri));
        });
    }
     ...
     */
    private validateLuisApplication(application: LuisApplicationWithRotatingKeys): void {
        if (!application.applicationId) {
            throw new Error(`Invalid \`applicationId\` value detected: ${ application.applicationId }\nPlease make sure your applicationId is a valid LUIS Application Id, e.g. "b31aeaf3-3511-495b-a07f-571fc873214b".`);
        }
        if (!application.endpointKeysToRotate || application.endpointKeysToRotate.length <= 0) {
            throw new Error(`Invalid \`endpointKey\` value detected: ${ application.endpointKeysToRotate }\nPlease make sure your endpointKey is a valid LUIS Endpoint Key, e.g. "048ec46dc58e495482b0c447cfdbd291".`);
        }
    }

    /**
     * Returns the endpointKey for the LuisClient with the endpointKey in the rotation for the current recognize() call
     */
    // private getCurrentLuisClientKey(): string {
    //     return (this.application as LuisApplicationWithRotatingKeys).endpointKeysToRotate[this.luisClientIndex];
    // }

    /**
     * Sets this.luisClientIndex to next match next luisClient. For rotating endpointKeys.
     */
    private setNextLuisClientIndex(): void {
        this.luisClientIndex = this.luisClientIndex + 1 < this.luisClients.length ? this.luisClientIndex + 1 : 0;
    }
}